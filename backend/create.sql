...
CREATE TABLE region ( id_reg INTEGER PRIMARY key, region VARCHAR );
...
CREATE TABLE producto (
    ...
    id_per_rut VARCHAR(9) pro_nom VARCHAR pro_id_cat INTEGER,
    pro_desc VARCHAR,
    pro_cant INTEGER,
    pro_val INTEGER,
    pro - img VARCHAR,
...
CREATE TABLE banco ( id_bco SERIAL PRIMARY key, bco_nom VARCHAR );

CREATE TABLE cuenta ( id_cta SERIAL PRIMARY key, tpo_cta VARCHAR );
...
VALUES (
        DEFAULT,
        'Banco de Chile, Banco Edwards, Citi, Atlas y CrediChile'
    ),
    (
        DEFAULT,
        'Banco Internacional'
    ...
    (
        DEFAULT,
        'Banco Santander (también Banefe)'
    ),
